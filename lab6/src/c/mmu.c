...
void *setup_page_tables(void *x0)
{
...
void walk(size_t *virt_pgd_p, size_t va, size_t pa, size_t flag)
{
...
void mmu_map_pages(size_t *virt_pgd_p, size_t va, size_t size, size_t pa, size_t flag)
{
    ...
    {
        walk(virt_pgd_p, va + s, pa + s, flag);
    }
}
...
void mmu_memfail_abort_handle(esr_el1_t *esr_el1)
{
        ...
        (esr_el1->iss & 0x3f) == TF_LEVEL3)
    {
        ...
        walk(PHYS_TO_VIRT(curr_thread->context.pgd), the_area_ptr->virt_addr + addr_offset, the_area_ptr->phys_addr + addr_offset, flag);
    }
    else
    {
